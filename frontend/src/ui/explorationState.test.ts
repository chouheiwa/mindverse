...
describe('probe exploration state', () => {
  test('moves through approach, inspection and scanning using one discriminated state', () => {
    ...
    expect(arrived.exploration).toMatchObject({ kind: 'probe-inspection', probe, scanComplete: false })
    ...
    expect(complete.exploration).toMatchObject({ kind: 'probe-inspection', probe, scanComplete: true })
  })
  ...
  test('ignores late callbacks from two probes and invalidates tokens on selection and exit', () => {
    ...
    expect(universeUiReducer(scanning, { type: 'probe-error', probeId: probe.id, token: 3 })).toBe(scanning)
    expect(universeUiReducer(scanning, { type: 'probe-error', probeId: otherProbe.id, token: 3 }).exploration)
      .toMatchObject({ kind: 'probe-inspection', star, probe: otherProbe, scanComplete: false })
  ...
  test('preserves a completed article link when a rescan command fails', () => {
    ...
    expect(universeUiReducer(rescan, { type: 'probe-error', probeId: probe.id, token: 6 }).exploration)
      .toMatchObject({ kind: 'probe-inspection', probe, scanComplete: true })
  })
})
